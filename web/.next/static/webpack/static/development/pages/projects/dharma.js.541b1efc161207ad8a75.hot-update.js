    ...
    p.draw = function () {
      ...
      if (H && W) {
        characters.map(function (_char, idx) {
          var countArray = _babel_runtime_corejs2_core_js_array_from__WEBPACK_IMPORTED_MODULE_0___default()(Array(_char.count).keys()); // p.scale(1, 0.5);


          countArray.map(function (duplicate, idxx) {
            var adjustedDuplicate = _char.count > 1 ? _char.count : duplicate + 1;
            var adjustedVerticalTranslation = H / adjustedDuplicate * (idxx + 1) * previousVerticalTranslation;
            var adjustedVerticalScale = characterSize / adjustedDuplicate;
            previousVerticalTranslation = H / 2.25;
            p.scale(1, adjustedVerticalScale);
            p.text(_char.letter, previousHorizontalTranslation(idx), adjustedVerticalTranslation);
          });
          p.scale(1, 1);
        });
      }
...
//# sourceMappingURL=dharma.js.541b1efc161207ad8a75.hot-update.js.map