    ...
    p.draw = function () {
      ...
      if (H && W) {
        characters.map(function (_char, idx) {
          ...
          var verticalDivisble = 2.25;
          var verticalPosition = H / verticalDivisble + H / verticalDivisble + H / verticalDivisble + H / verticalDivisble; // p.scale(1, 0.5);

          countArray.map(function (duplicate, idxx) {
            // let adjustedDuplicate = char.count > 1 ? char.count : duplicate + 1;
            var verticalScale = _char.count > 0 ? H / _char.count / H : 1;
            var duplicateVerticalPosition = verticalPosition;
            verticalPosition = duplicateVerticalPosition;
            p.push();
            p.scale(1, verticalScale);
            p.text(_char.letter, previousHorizontalTranslation(idx), verticalPosition);
            p.pop();
          });
        });
      }
...
//# sourceMappingURL=dharma.js.0de6e0f066414291a79c.hot-update.js.map